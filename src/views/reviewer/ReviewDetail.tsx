    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 400 }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontWeight: 700, fontSize: 18, color: '#2a3439', margin: '0 0 6px' }}>
            {decision === 'approve' ? 'Submission Approved' : 'Submission Rejected'}
          </p>
          <p style={{ fontSize: 13, color: '#566166', margin: 0 }}>Loading next submission...</p>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          <div>
            <h1 style={{ fontWeight: 800, fontSize: 20, color: '#2a3439', margin: 0 }}>Reviewing: <span style={{ color: '#2d6197' }}>{sub.word}</span></h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e1e9ee', overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #e1e9ee', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
              ...
              <a href={sub.postUrl} target="_blank" rel="noreferrer"
                style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#2d6197', fontWeight: 600, textDecoration: 'none' }}>
                Open on Reddit <ExternalLink size={12} />
              </a>
            </div>
            <div style={{ padding: '20px' }}>
              {/* Reddit-style card */}
              <div style={{ background: '#f7f9fb', borderRadius: 10, padding: '16px 18px', border: '1px solid #e1e9ee' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 11, color: '#717c82' }}>· Posted by u/throwaway_user · {sub.postedAt}</span>
                </div>
                <p style={{ fontSize: 14, color: '#2a3439', lineHeight: 1.65, margin: 0 }}>
                  {highlightWord(sub.postText, sub.word)}
                </p>
                <div style={{ display: 'flex', gap: 16, marginTop: 12 }}>
                  <span style={{ fontSize: 12, color: '#566166' }}>▲ {sub.upvotes} upvotes</span>
                  <span style={{ fontSize: 12, color: '#566166' }}>💬 {sub.comments} comments</span>
                </div>
              </div>
              <p style={{ fontSize: 11, color: '#717c82', margin: '10px 0 0' }}>
          ...
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e1e9ee', padding: '20px' }}>
            <p style={{ fontWeight: 700, fontSize: 14, color: '#2a3439', margin: '0 0 14px' }}>Automated Checks</p>
          ...
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e1e9ee', padding: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10, marginBottom: 16 }}>
              ...
              <button onClick={() => setDecision('reject')}
                style={{ padding: '14px', borderRadius: 12, border: `2px solid ${decision === 'reject' ? '#EF4444' : '#e1e9ee'}`, background: decision === 'reject' ? '#FEF2F2' : '#fff', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 14, fontWeight: 700, color: decision === 'reject' ? '#991B1B' : '#374151', transition: 'all 0.15s' }}>
                <XCircle size={18} color={decision === 'reject' ? '#EF4444' : '#717c82'} /> Reject
              </button>
            </div>
            ...
            {decision === 'reject' && (
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {rejectReasons.map(r => (
                    <label key={r} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13, color: rejectReason === r ? '#2a3439' : '#566166' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Add a note for your records..."
                style={{ width: '100%', padding: '10px 12px', fontSize: 13, border: '1.5px solid #e1e9ee', borderRadius: 10, fontFamily: 'inherit', outline: 'none', resize: 'vertical', minHeight: 70, boxSizing: 'border-box' }}
            ...
            <button onClick={handleSubmit}
              ...
              style={{ width: '100%', padding: '12px', background: !decision || (decision === 'reject' && !rejectReason) ? '#e1e9ee' : decision === 'approve' ? '#10B981' : '#EF4444', color: !decision || (decision === 'reject' && !rejectReason) ? '#717c82' : '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: !decision || (decision === 'reject' && !rejectReason) ? 'not-allowed' : 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {/* Word Info */}
          <div style={{ background: 'linear-gradient(135deg, #2d6197, #1a4670)', borderRadius: 14, padding: '18px 20px', color: '#fff' }}>
            <p style={{ fontSize: 10, color: '#d2e4ff', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 8px' }}>Word to Find</p>
            <p style={{ fontSize: 24, fontWeight: 800, margin: '0 0 6px' }}>{sub.word}</p>
            <p style={{ fontSize: 11, color: '#d2e4ff', margin: '0 0 10px', fontStyle: 'italic' }}>{sub.partOfSpeech}</p>
            <p style={{ fontSize: 13, color: '#d2e4ff', margin: 0, lineHeight: 1.5 }}>{sub.definition}</p>
          </div>

          {/* Contributor Info */}
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e1e9ee', padding: '18px 20px' }}>
            <p style={{ fontWeight: 700, fontSize: 13, color: '#2a3439', margin: '0 0 12px' }}>Contributor</p>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, #2d6197, #1a4670)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <User size={16} color="#fff" />
              </div>
              <div>
                <p style={{ fontWeight: 600, fontSize: 14, color: '#2a3439', margin: 0 }}>{sub.contributor}</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: 8 }}>
              {[
                ...
                <div key={s.label} style={{ padding: '10px', background: '#f7f9fb', borderRadius: 8, textAlign: 'center' }}>
                  <p style={{ fontSize: 16, fontWeight: 800, color: s.color, margin: 0 }}>{s.val}</p>
                  <p style={{ fontSize: 10, color: '#717c82', margin: 0 }}>{s.label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Submission Info */}
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e1e9ee', padding: '18px 20px' }}>
            <p style={{ fontWeight: 700, fontSize: 13, color: '#2a3439', margin: '0 0 12px' }}>Submission Details</p>
            {[
              ...
              <div key={d.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid #f7f9fb' }}>
                <span style={{ fontSize: 12, color: '#717c82' }}>{d.label}</span>
          ...
          {/* Queue Position */}
          <div style={{ background: '#f7f9fb', borderRadius: 14, border: '1px solid #e1e9ee', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 10 }}>
            <Clock size={16} color="#717c82" />
